...
export default function JoinPage() {
  ...
  async function handleSubmit() { await submitNow('manual') }
          ...
          {result.feedback.map(f => (
            <div key={f.qIndex} className="border rounded-xl p-4 mb-4">
              ...
              <p className="text-sm mt-2">
                You selected {String.fromCharCode(65 + f.chosenIndex)} — {f.correct ? 'correct' : 'incorrect'}. Correct Answer {String.fromCharCode(65 + f.correctIndex)}.
              </p>
            </div>
          ))}
          <button
            className="btn-primary"
            onClick={() => { setRole('student'); navigate('/student') }} // ✅ force the student role before navigating
          >
            Back to my results
          </button>
        </div>
