...
function loadDB(): Student[] {
  try { return JSON.parse(localStorage.getItem(LS_STUDENT_DB) || '[]') as Student[] } catch { return [] }
}
function saveDB(db: Student[]) { localStorage.setItem(LS_STUDENT_DB, JSON.stringify(db)) }
